...
void potentiometers() {
  ...
  for (int i = 0; i < N_POTS; i++) {  // Loops through all the potentiometers
    ...
    potMidiCState[i] = map(potCState[i], faderMin, faderMax, 0, 127);  // Maps the reading of the potCState to a value usable in midi
...
#ifdef USING_MOTORIZED_FADERS
    PBVal[i] = map(potCState[i], faderMin, faderMax, 0, 16383);
      ...
      if (potMidiPState[i] != potMidiCState[i]) {
        ...
        if (isTouched[i]) {
          ...
          if (faderPos[i] != faderPPos[i]) {
            ...
            Serial.print("faderPos pot: ");
            Serial.println(faderPos[i]);
        ...
        // MOTORRRRRRR
        pitchBend(selectedTrack, PBVal[i]);
        MidiUSB.flush();
